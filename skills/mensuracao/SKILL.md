...
Este módulo tem uma referência opcional: a etapa `mensuracao/gtm` (`references/gtm.md`), planejada só quando o rastreamento passa por um container do Google Tag Manager e a rota recebe `--requires-gtm-audit`. A integração nunca publica uma versão do GTM.
