...
5. **Se houver correção acionável**, gerar item de change set no formato de `12-optimization-change-set` (alvo = tag/trigger/variável, `target_ref` = path do recurso, antes/depois, justificativa, evidência = snapshot, risco, reversão). Marcar `platform` do item como a plataforma de mídia que a tag serve (`meta` ou `google_ads`), não "gtm" — GTM não é uma plataforma de mutação no schema do dossiê.
