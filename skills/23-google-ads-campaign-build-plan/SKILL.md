...
5. Gerar change set versionado com itens Google Ads marcados `manual_only`.
6. Atualizar dossiê para `proposed` somente quando houver mutação proposta; planejamento puro termina `analysis_only`.

## Gate

Não chamar ferramenta de escrita. Plano completo não é aprovação. Campo ausente fica `blocked`; configuração cuja disponibilidade não foi validada exige instrução para conferência manual.
