...
- **Armazenamento**: matrizes numpy locais em `knowledge/.vector-index/` — não é um banco de dados hospedado nem um serviço externo. Artefato derivado, regenerável a partir dos `.md` fonte via `scripts/build_knowledge_vector_index.py`, e versionado no Git (~2.6MB) para o time receber o RAG pronto no clone/pull; `validate_repository.py` compara o hash do manifest com os `.md` fonte para detectar desatualização em vez de deixá-la passar silenciosa.
