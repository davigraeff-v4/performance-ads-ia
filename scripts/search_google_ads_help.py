...
def score_article(query, article):
    return _score_article(query, article, tokens=tokens, category_bonus=False)


def body_signal(query, article):
    return _body_signal(query, article, tokens=tokens)
...
def main() -> int:
    ...
    parser.add_argument(
        "--platform",
        required=True,
        choices=["meta", "google_ads"],
        help="plataforma já resolvida pelo roteador",
    )
    ...
    normalized_query_tokens = set(normalize(query).split())
    platform_conflict = (
        args.platform == "google_ads"
        and {"meta", "ads"}.issubset(normalized_query_tokens)
    )
    if args.platform != "google_ads" or platform_conflict:
        reason = "platform_not_google_ads" if args.platform != "google_ads" else "query_platform_conflict"
        payload = {
            "query": query,
        ...
        if args.as_json:
            ...
            print(f"Consulta fora do escopo da base Google Ads: {reason}")
        return 0
    ...
    if not args.title_only and args.mode != "lexical" and (args.mode == "vector" or top_title_score < 75):
        candidates = vector_candidates(query, "google_ads")
        if candidates is None:
            print("aviso: índice vetorial indisponível para google_ads; usando busca lexical apenas.", file=sys.stderr)
        else:
            vector_used = True
            combined = []
            for score, article in ranked:
    ...
    results = [
        ...
        for score, article, signal, section in combined[: args.limit]
    ]
    payload = {
        "query": query,
        "platform": "google_ads",
    ...
    print(f"Consulta Google Ads: {query}")
    print(f"Base: {len(articles)} artigos | Busca: {stage} | Resultados: {len(results)}")
