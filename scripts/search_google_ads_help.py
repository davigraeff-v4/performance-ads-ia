...
def load_articles() -> list[Article]:
    """Carrega somente o INDEX.md; os corpos ficam fechados nesta etapa."""
    index_path = BASE / "INDEX.md"
    if not index_path.is_file():
        raise FileNotFoundError(f"índice ausente: {index_path.relative_to(ROOT)}")

    text = index_path.read_text(encoding="utf-8")
    date_match = re.search(r"iniciado em (\d{4}-\d{2}-\d{2})", text)
    ...
    if not articles:
        raise ValueError(f"nenhum artigo encontrado em {BASE.relative_to(ROOT)}")
    return articles
...
def body_signal(query: str, article: Article) -> float:
    ...
    return min(32.0, 32.0 * hits / max(len(query_tokens), 1))
...
def main() -> int:
    ...
    results = [
        ...
        for score, article in ranked[: args.limit]
    ]
    payload = {
        "query": query,
        "platform": "google_ads",
